...
if(!empty($arResult['ERRORS']['FATAL'])):
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?= ($arResult['MODE_CLASSES'] !== '' ? $arResult['MODE_CLASSES'] : ''); ?>">
		...
		<div class="header-search">
			<svg class="header-search__icon" aria-hidden="true" width="19" height="19">
				<use xlink:href="<?= SITE_TEMPLATE_PATH ?>/img/sprite.svg#search1"></use>
			</svg>
			<div class="header-search__inner <?=$dropDownBlock?> bx-ui-sls-input-block">
				<span class="dropdown-icon" style="display: none;"></span>
				<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="header-search__input dropdown-field" />
