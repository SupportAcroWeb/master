...
if ((string)$request->get('ORDER_ID') !== '') {
    ...
    <form action="<?= POST_FORM_ACTION_URI ?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form"
        ...
        <div id="bx-soa-order" class="block-order__content form">

			<!--	MAIN BLOCK	-->
            <div class="block-order__body block1 bx-soa">
                ...
                <!--	BUYER BLOCK	-->
                <div class="block-order__column" data-props-group="buyer">
                ...
                <!--	ORGANIZATION BLOCK	-->
                <div class="block-order__column" data-props-group="organization">
                    <div class="block-order__title">
                        данные организации
                    </div>
                    <div class="form">
				...
				<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active">
                    ...
                    <div id="bx-soa-delivery" data-visited="false"
                         class="bx-soa-section bx-active bx-selected" <?= ($hideDelivery ? 'style="display:none"' : '') ?>>
						<div class="bx-soa-section-title-container">
                    ...
                    <div class="addres-block-order courier-delivery _active" data-props-group="external_delivery" style="display:none">
                        <div class="form">
                            <div class="form-grid1">
						</div>
					</div>
                ...
                <!--	ADDITIONAL BLOCK	-->
                <div class="block-order__column" data-props-group="additional">
                    <div class="block-order__title">
					...
					<!--	BASKET ITEMS BLOCK	-->
                <div id="bx-soa-basket" data-visited="false" class="block-order__column bx-soa-section bx-active">
                    <div class="block-order__title">
                        товары в заказе
						</div>
				...
				<div id="bx-soa-orderSave">
					<div class="checkbox duble_check">
						<?php
						$APPLICATION->IncludeComponent(
							'bitrix:main.userconsent.request',
							'',
							[
								'ID' => $arParams['USER_CONSENT_ID'] ?: 2,
								'IS_CHECKED' => 'N',
								'IS_LOADED' => $arParams['USER_CONSENT_IS_LOADED'] ?? 'Y',
								'AUTO_SAVE' => 'N',
			...
			<!--	SIDEBAR BLOCK	-->
             <div id="bx-soa-total" class="details-block-cart bx-soa-sidebar"></div>
		</div>
	</form>
	...
	// Обработчик события триггера валидации для оформления заказа
	$(document).on('order', function(event, data) {
		if (data && data.form) {
			...
			// Отключаем стандартную валидацию Битрикса и отправляем форму
			if (submitBtn) {
				// Временно отключаем обработчик клика
				var oldOnclick = submitBtn.onclick;
				submitBtn.onclick = null;

				// Отправляем форму стандартным способом Битрикса
				form.submit();

				// Возвращаем обработчик обратно
				setTimeout(function() {
					submitBtn.onclick = oldOnclick;
				}, 100);
			}
		}
	});
