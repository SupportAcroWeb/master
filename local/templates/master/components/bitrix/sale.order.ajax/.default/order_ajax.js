...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
        ...
        editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container block-order-radio'}}),
				paySystemItemNode, i;
        ...
        createPaySystemItem: function (item) {
			var checked = item.CHECKED == 'Y',
				...
				title, titleSpan, label, itemNode, checkClass, checkboxText;

			logoNode = BX.create('DIV', {props: {className: 'bx-soa-pp-company-image radio__visual'}});
			...
			label = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-graf-container radio'},
				children: [
					BX.create('INPUT', {
						props: {
							...
							name: 'PAY_SYSTEM_ID',
							type: 'checkbox',
							className: 'bx-soa-pp-company-checkbox radio__input',
							value: paySystemId,
							checked: checked
						}
					}),
            ...
            if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				titleSpan = BX.create('DIV', {props: {className: 'checkbox-text__titles'}, text: item.NAME});
				title = BX.create('DIV', {props: {className: 'bx-soa-pp-company-smalltitle checkbox-text__label'}, children: [titleSpan]});
			}
			...
			checkboxText = BX.create('DIV', {
				props: {className: 'checkbox-text' + checkClass},
        ...
        editDeliveryItems: function (deliveryNode) {
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container block-order-radio'}}),
				deliveryItemNode, k;
        ...
        createDeliveryItem: function (item) {
			var checked = item.CHECKED == 'Y',
				...
				logotype, titleSpan, label, titleSpanName, titleSpanDescription, title, itemNode, logoNode, priceSpan, checkClass, checkboxText;
            ...
            if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined') {
				...
				priceSpan = BX.create('DIV',
					{
						props: {className: 'checkbox-text__price'},
						html: typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined'
							? item.DELIVERY_DISCOUNT_PRICE_FORMATED
							: item.PRICE_FORMATED
					}
			...
			label = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-graf-container radio'
            ...
            if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
				titleSpanDescription = BX.create('DIV', {props: {className: 'checkbox-text__subtitle'}, text: item.DESCRIPTION});
				titleSpanName = BX.create('DIV', {props: {className: 'checkbox-text__titles'}, text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME});
				titleSpan = BX.create('DIV', {props: {className: 'checkbox-text__titles'}, text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME, children: [titleSpanName, titleSpanDescription]});
				title = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-smalltitle checkbox-text__label'},
					children: [titleSpan, priceSpan]
				});
			}
			checkClass = '';
			if (checked) {
		...
		var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer'}}),
            ...
            while (group = groupIterator()) {
                ...
                // Определяем контейнер на основе параметров компонента
                var targetContainer = propsItemsContainer;
                
                if (groupId == this.params.BUYER_PROPS_GROUP_ID && buyerBlockContainer) {
                    targetContainer = buyerBlockContainer;
                } else if (groupId == this.params.ORGANIZATION_PROPS_GROUP_ID && organizationBlockContainer) {
                    targetContainer = organizationBlockContainer;
                } else if (groupId == this.params.ADDRESS_PROPS_GROUP_ID && addressBlockContainer) {
                    targetContainer = addressBlockContainer;
                } else if (groupId == this.params.ADDITIONAL_PROPS_GROUP_ID && additionalBlockContainer) {
                    targetContainer = additionalBlockContainer;
                } else if (groupId == this.params.EXTERNAL_DELIVERY_PROPS_GROUP_ID && externalDeliveryBlockContainer) {
