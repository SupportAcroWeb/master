...
>
    ...
    <?
    $key = array_search('REASON', array_column($arResult['QUESTIONS'], 'SID'));
    $reasonQuestion = $key !== false ? $arResult['QUESTIONS'][$key] : null;
    if ($reasonQuestion):
    ?>
        <div class="form">
            <div class="form-grid1">
                <div class="form-grid1__row form-grid1-top ">
                    <div class="form-grid1__title">причина обращения</div>
                </div>
                <div class="form-grid1__row">
                    <select data-select class="ts-wrapper_wide select2" required name="<?= 'form_text_' . $reasonQuestion['ID'] ?>">
                        <option value="Прочие вопросы">Прочие вопросы</option>
                        <option value="Сложности с заказом">Сложности с заказом</option>
                        <option value="Хочу купить товар">Хочу купить товар</option>
                        <option value="Нужна помощь в выборе">Нужна помощь в выборе</option>
                    </select>
                </div>
            </div>
        </div>
    <?php endif; ?>
    <div class="form-grid1 form">
        <div class="form-grid1__row form-grid1-top">
            <div class="form-grid1__title">контактные данные</div>
        </div>
        <?php foreach ($arResult['QUESTIONS'] as $question):
            if ($question['SID'] === 'REASON') {
                continue;
            } ?>
            <div class="form-grid1__row<?= $question['SID'] === 'PHONE' || $question['SID'] === 'EMAIL' ? ' form-grid1__row_2' : '' ?>">
                <div class="form-group1">
                    <?php if ($question['SID'] === 'MESSAGE'): ?>
