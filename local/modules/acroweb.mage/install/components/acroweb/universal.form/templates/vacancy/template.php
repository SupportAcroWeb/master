...
<div class="hystmodal__inner">
    ...
    <form id="<?= $arResult['FORM_UNIQUE_ID'] ?>"
          ...
          data-validate
          data-onsubmit-trigger="vacancyFormSubmit<?= $arResult['PREFIX_UNIQUE'] ?>"
    >
        <?= bitrix_sessid_post() ?>
        <input type="hidden" name="WEB_FORM_ID" value="<?= $arResult['FORM']['ID'] ?>">
        <div class="form-grid1">
            <?php foreach ($arResult['QUESTIONS'] as $question): ?>
                <div class="form-grid1__row" <?= $question['INPUT_TYPE'] != 'hidden' ?: 'style="display:none"' ?>>
                    <div class="form-group">
                        <?php if ($question['SID'] === 'FILE'): ?>
                            <label class="input-file">
                                <span class="input-file__label" data-label="<?= $question['TITLE'] ?>"><?= $question['TITLE'] ?></span>
                                <input data-type="file" class="input-file__input" type="file" name="<?= $question['INPUT_NAME'] ?>" id="<?= $question['INPUT_ID'] ?>" <?= $question['REQUIRED'] === 'Y' ? 'required' : '' ?>>
...
<script>
    BX.ready(function() {
        class VacancyForm<?= $arResult['PREFIX_UNIQUE'] ?> extends UniversalForm {
            onSuccess(message) {
        ...
        let universalForm<?= $arResult['PREFIX_UNIQUE'] ?> = new VacancyForm<?= $arResult['PREFIX_UNIQUE'] ?>(<?= Json::encode($arResult['JS_PARAMS']) ?>);

        $(document).on('vacancyFormSubmit<?= $arResult['PREFIX_UNIQUE'] ?>', function(event, form) {
