...
class PageProp {
	 ...
	 */
	public static function OnEndBufferContent_Handler(&$strContent) {
		...
		if(in_array($GLOBALS['APPLICATION']->GetCurPage(), ['/bitrix/admin/public_file_property.php','/bitrix/admin/public_folder_edit.php'])) {
			static::GetTypes();
			$strContent = preg_replace_callback('#<tr style.*?>[\s]{0,}<td class="bx-popup-label bx-width30">(.*?):</td>[\s]{0,}<td>(.*?)</td>[\s]{0,}</tr>#is'.(Helper::IsUtf()?'u':''),function($matches){
				return static::modifyPagePropsDialogReplace($matches);
			}, $strContent);
		}
	}
	 ...
	 */
	public static function modifyPagePropsDialogReplace($Matches){
		$strData = $Matches[2];
		$PropertyID = false;
		$PropertyCode = false;
		$PropertyValue = false;
	 ...
	 */
	public static function GetTypes($Force=false) {
		$arResult = array();
	 ...
	 *	Показ настроек
	 */
	public static function ShowSettings($PropertyCode, $SiteId, $PropertyType=false) {
	 ...
	 *	Показ настроек
	 */
	public static function SaveSettings($PropertyCode, $PropertySite, $PropertyType, $arPost) {
		$PropertyType = trim($PropertyType);
		$arTypes = static::GetTypes();
		if ($PropertyType!==false && is_array($arTypes[$PropertyType])) {
			$ClassName = $arTypes[$PropertyType]['CLASS'];
	 ...
	 */
	public static function GetFilter($PropertyCode, $SiteID=false) {
		$arResult = array();
	 ...
	 */
	public static function ShowControls($PropertyCode, $PropertyID, $PropertyValue, $SiteID) {
		global $DB;
