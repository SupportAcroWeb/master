...
class Options {
	...
	protected $arOldValues = [];
	protected $arNewValues = [];
	...
	public function __construct($strModuleId, $arTabs, $arParams=null){
		...
		#
		foreach($arTabs as $intTabIndex => $arTab){
			...
			$arTabs[$intTabIndex] = $arTab;
		}
		#
		$this->obTabControl = new \CAdminTabControl($this->getTabControlId(), $arTabs);
		#
		\CJSCore::init(['ajax', 'jquery2', 'jquery3', 'wdupopup']);
		$APPLICATION->addHeadScript('/bitrix/js/'.WDU_MODULE.'/helper.js');
		$APPLICATION->addHeadScript('/bitrix/js/'.WDU_MODULE.'/wdu_popup.js');
		#
		foreach($arTabs as $intTabIndex => $arTab){
			if(is_array($arTab['OPTIONS'])){
				$arTab['OPTIONS_RAW'] = $arTab['OPTIONS'];
	...
	/**
	 *	Handler AJAX-request from options
	 */
		...
		foreach($arOptions as $strFile){
			...
			if(is_file($strFilename)){
				...
				if(!isset($arNewOptions['NAME']) || !strlen($arNewOptions['NAME'])){
					...
					$arNewOptions['INFO'] = Helper::getMessage(static::LANG.'INFO_'.$strGroupLangCode);
				}
				foreach($arNewOptions['ITEMS'] as $strOption => &$arOption){
	...
	public function save(){
		...
		if($this->bUpdate || $this->bApply){
			...
			foreach($this->arTabs as $strTab => $arTab){
				if(is_array($arTab['OPTIONS'])){
					foreach($arTab['OPTIONS'] as $arGroup){
						if(is_array($arGroup['ITEMS'])){
							foreach($arGroup['ITEMS'] as $strOption => $arOption) {
								$this->arOldValues[$strOption] = $this->getOptionValue($strOption);
							}
						}
					}
				}
			...
			foreach($this->arTabs as $strTab => $arTab){
				if(is_array($arTab['OPTIONS'])){
					foreach($arTab['OPTIONS'] as $arGroup){
						if(is_callable($arGroup['CALLBACK_BEFORE_SAVE'])){
							call_user_func_array($arGroup['CALLBACK_BEFORE_SAVE'], [$this, &$arTab, $strTab, $arGroup]);
						}
						if(is_array($arGroup['ITEMS'])){
							foreach($arGroup['ITEMS'] as $strOption => $arOption) {
								$strValue = $this->arPost[$strOption];
								$this->arNewValues[$strOption] = $strValue;
								...
								if(is_callable($arOption['CALLBACK_AFTER_SAVE'])){
									call_user_func_array($arOption['CALLBACK_AFTER_SAVE'], [$this, $strValue, $arOption, $strOption, 
										$strOptionId]);
								}
							}
							if(is_callable($arGroup['CALLBACK_AFTER_SAVE'])){
								call_user_func_array($arGroup['CALLBACK_AFTER_SAVE'], [$this, &$arTab, $strTab, $arGroup]);
							}
						}
					}
				}
						...
						if(is_array($arGroup['ITEMS'])){
							foreach($arGroup['ITEMS'] as $strOption => $arOption) {
								if(is_callable($arOption['CALLBACK_SAVE'])){
									$arOption['VALUE_OLD'] = $this->arOldValues[$strOption];
									$arOption['VALUE_NEW'] = $this->arNewValues[$strOption];
									$strOptionId = $this->getOptionPrefix($strOption);
		...
		elseif($this->bReset){
			Helper::removeAllOptions($this->strModuleId);
		}
		#
		ob_start();
	...
	/**
	 *	Get old option values (before save)
	 */
	public function getOldValues($arValuesSelected=[]){
		return static::getValuesSelected($this->arOldValues, $arValuesSelected);
	}
	
	/**
	 *	Get new option values (after save)
	 */
	public function getNewValues($arValuesSelected=[]){
		return static::getValuesSelected($this->arNewValues, $arValuesSelected);
	}
	
	/**
	 *	Get selected values (arg2) from array (arg1)
	 */
	public static function getValuesSelected(array $arValuesAll, $arValuesSelected){
		if(!is_array($arValuesSelected)){
			$arValuesSelected = is_string($arValuesSelected) && strlen($arValuesSelected) ? [$arValuesSelected] : [];
		}
		if(empty($arValuesSelected)){
			return $arValuesAll;
		}
		$arValuesSelected = array_flip($arValuesSelected);
		return array_intersect_key($arValuesAll, $arValuesSelected);
	}
				...
				foreach($arTab['OPTIONS'] as $strGroup => $arGroup){
					if(is_array($arGroup['ITEMS'])){
						foreach($arGroup['ITEMS'] as $strOption => $arOption){
							...
							$this->arTabs[$strTab]['OPTIONS'][$strGroup]['ITEMS'][$strOption]['VALUE'] = $strValue;
						}
					}
				}
			}
	...
	public function displayOptions($arOptions){
		print Helper::includeFile(WDU_MODULE, 'options/template.php', [
			'OPTIONS' => $arOptions,
			'THIS' => $this,
		]);
	}
