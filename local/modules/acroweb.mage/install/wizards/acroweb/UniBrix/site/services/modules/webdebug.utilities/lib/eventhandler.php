...
class EventHandler {
	...
	public static function onBeforeProlog(){
		Adv::onBeforeProlog();
		AdminAuthNotifier::onBeforeProlog();
	}
	
	public static function onProlog(){
		# Use jquery.select2 for modules list
		...
		# jQuery select2 for custom pages
		if(defined('ADMIN_SECTION') && ADMIN_SECTION === true) {
			if(Helper::getOption(WDU_MODULE, 'use_select2_for_custom_admin_pages') == 'Y') {
				JsHelper::applyCustomSelect2();
			}
		}
		#
	...
	public static function onEpilog(){
		...
		# Add current template path in edit file popup
		...
		# Auto-expand popup-menus (like Windows 10)
		if(Helper::getOption(WDU_MODULE, 'popup_expand_on_top_edge') == 'Y'){
			if($GLOBALS['USER']->isAdmin()){
				$GLOBALS['APPLICATION']->addHeadString('
					<script>
					if(BX){
						BX.addCustomEvent("onWindowDrag", function(){
							let rect = this.DIV.getBoundingClientRect(),
								time = new Date().getTime();
							if(rect.top <= 0 && this.SETTINGS.resizable && !this.bExpanded){
								if(!this.wduTime || time - this.wduTime > 500){
									this.__expand();
									this.wduTime = time;
								}
							}
							else if(rect.top > 0 && this.bExpanded){
								this.__expand();
							}
						});
		...
		# Accessibility
		if(Helper::getUserOption(WDU_MODULE, 'include_accessibility_js') == 'Y'){
			\Bitrix\Main\Page\Asset::getInstance()->addJs('/bitrix/js/'.WDU_MODULE.'/accessibility.js');
		}
	}
	...
	public static function onEndBufferContent(&$strContent){
		...
		if(Helper::getOption(WDU_MODULE, 'iblock_show_iblock_prop_meta')=='Y'){
			IBlockHelper::editFormAddIdCode($strContent);
		}
	}
	...
	public static function onAdminTabControlBegin(&$obTabControl){
		...
		IBlockHelper::hideEditTabs($obTabControl);
	}
	
	public static function onBuildGlobalMenu(&$arGlobalMenu, &$arMenuItems){
		MenuManager::onBuildGlobalMenu($arGlobalMenu, $arMenuItems);
	}
	...
	public static function onBeforeEventSend(&$arFields, &$eventMessage, $context=null){
	...
	public static function OnAfterUserAuthorize($arUser){
		AdminAuthNotifier::OnAfterUserAuthorize($arUser);
	}
