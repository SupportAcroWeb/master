...
class Cli {
	 ...
	 *	Wrapper for exec()
	 */
	public static function exec($strCommand, $bAsText=false){
		$arExecResult = [];
		if(static::isExec()){
			exec($strCommand, $arExecResult);
		}
		if($bAsText){
			if(count($arExecResult) == 1 && !strlen($arExecResult[0])){
				unset($arExecResult[0]);
			}
			$arExecResult = implode(PHP_EOL, $arExecResult);
		}
		if(Cli::isWindows()){
			$arExecResult = Helper::convertEncoding($arExecResult, 'CP866', Helper::isUtf() ? 'UTF-8' : 'CP1251');
		}
		return $arExecResult;
	}
