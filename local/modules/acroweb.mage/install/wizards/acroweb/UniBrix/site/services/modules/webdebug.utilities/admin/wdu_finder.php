...
if(strlen($arGet['wdu_ajax_option'])){
	...
	switch($arGet['wdu_ajax_option']){
		case 'ajax_search':
			...
			$strFolder = strlen($arState['FOLDER']) && is_dir($_SERVER['DOCUMENT_ROOT'].$arState['FOLDER']) ? $_SERVER['DOCUMENT_ROOT'].$arState['FOLDER'] : $_SERVER['DOCUMENT_ROOT'];
			$mResult = $obFinder->scan($strFolder);
