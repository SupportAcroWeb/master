...
function wduSelect2(select, config){
	return $(select).each(function(){
		...
		if(!select2.hasClass('select2-hidden-accessible')){
			...
			if(configItem.withIcons){
				configItem.templateResult = function(item){
				...
				configItem.templateSelection = function(item){
					if(!item.id) {
						return item.text;
					}
					var option = $(item.element);
			...
			if(configItem.extConfig){
				configItem = $.extend(configItem, configItem.extConfig);
			}
			select2.select2(configItem);
