...
# Handler
# Use example:
# addEventHandler('webdebug.utilities', 'OnDisplayPropSorterTemplate', function(&$arResult, &$arParams, &$strTemplateName, $obComponent){
# 	$arResult['PROPS_GROUPS']['GROUP_3']['NAME'] .= ' Test!';
# 	$strTemplateName = 'dotted';
# });
$strTemplateName = $this->getTemplateName();
foreach(getModuleEvents(WDU_MODULE, 'OnDisplayPropSorterTemplate', true) as $arEvent) {
	executeModuleEventEx($arEvent, [&$arResult, &$arParams, &$strTemplateName, $this]);
}
if($strTemplateName != $this->getTemplateName()){
	$this->setTemplateName($strTemplateName);
}
