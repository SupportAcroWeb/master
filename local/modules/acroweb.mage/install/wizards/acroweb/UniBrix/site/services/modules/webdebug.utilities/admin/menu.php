...
if(\Bitrix\Main\Loader::includeModule($strModuleId) && $APPLICATION->getGroupRight($strModuleId)>='R') {
	...
	// Shell
	$arSubmenu[] = [
		'text' => Helper::getMessage('WDU_MENU_SHELL'),
		'more_url' => [],
		'url' => 'wdu_shell.php?lang='.LANGUAGE_ID,
		'icon' => 'wdu_icon_shell',
	];
	
	// Backups
	$arSubmenu[] = [
		'text' => Helper::getMessage('WDU_MENU_BACKUPS'),
		'more_url' => [],
		'url' => '/bitrix/admin/settings.php?lang='.LANGUAGE_ID.'&mid='.$strModuleId.'&wdu_tabs_active_tab=backups',
		'icon' => 'wdu_icon_backups',
	];
