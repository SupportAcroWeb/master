...
if($this->StartResultCache())
{
	...
	else
	{
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			if (!empty($arParams["SECTION_FIELDS"]) && is_array($arParams["SECTION_FIELDS"])) {
				foreach ($arParams["SECTION_FIELDS"] as $field) {
					if (isset($arSection[$field])) {
						$sectionData[$field] = $arSection[$field];
						$sectionData["~".$field] = $arSection["~".$field];
					}
				}
			}
			
			$arResult["SECTIONS"][] = $sectionData;
...
foreach($arResult["SECTIONS"] as $arSection)
{
	...
	// Добавляем дополнительные поля в параметры
	if (!empty($arParams["SECTION_FIELDS"]) && is_array($arParams["SECTION_FIELDS"])) {
		foreach ($arParams["SECTION_FIELDS"] as $field) {
			if (isset($arSection[$field])) {
				$menuParams[$field] = $arSection[$field];
			}
		}
	}
