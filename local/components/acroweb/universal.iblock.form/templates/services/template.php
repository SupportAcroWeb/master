...
<div class="hystmodal__inner">
    ...
    <form id="<?= $formId ?>"
          ...
          data-validate
          data-onsubmit-trigger="servicesFormSubmit_<?= CUtil::JSEscape($formId) ?>"
    >
        <?= bitrix_sessid_post() ?>
            ...
            <?php foreach ($arResult['FIELDS'] as $field): ?>
                <div class="form-grid1__row">
                    <div class="form-group">
                        <label class="form-group__label<?= $field['IS_REQUIRED'] === 'Y' ? ' form-group__label_req' : '' ?>" for="<?= $field['CODE'] ?>" <?= $field['CODE'] == 'SERVICES_ID' ? 'style="display:none"' : '' ?>><?= $field['NAME'] ?></label>
                        <?php
                        switch ($field['PROPERTY_TYPE']) {
                            case 'S': // Строка
                                if ($field['USER_TYPE'] === 'HTML') {
                                    ...
                                    echo '<input ' . ($field['CODE'] === 'EMAIL' ? 'data-type="email"' : '') . ' ' . ($field['CODE'] === 'SERVICES_ID' ? 'type="hidden" data-modal-service="id"' : 'type="text"') . ' ' . ($field['CODE'] === 'SERVICES_NAME' ? 'data-modal-service="position" readonly' : '') . ' id="' . $field['CODE'] . '" name="PROPERTY[' . $field['CODE'] . ']" ' . ($field['CODE'] === 'PHONE' ? 'data-type="phone" data-mask="phone" ' : '') . ' class="field-input1 form-group__field"' . ($field['IS_REQUIRED'] === 'Y' ? ' required' : '') . '>';
                                }
                                break;
                            case 'N': // Число
...
<script>
    BX.ready(function() {
        class ServicesForm_<?= CUtil::JSEscape($formId) ?> extends UniversalIblockForm {
            onSuccess(message) {
        ...
        let servicesForm_<?= CUtil::JSEscape($formId) ?> = new ServicesForm_<?= CUtil::JSEscape($formId) ?>({
        ...
        $(document).on('servicesFormSubmit_<?= CUtil::JSEscape($formId) ?>', function(event, formData) {
            if (formData.form.id === '<?= CUtil::JSEscape($formId) ?>') {
                servicesForm_<?= CUtil::JSEscape($formId) ?>.submitForm(formData.form);
            }
        });
    });
</script>