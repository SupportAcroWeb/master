...
<div class="hystmodal__inner">
    ...
    <form id="<?= $arResult['FORM_UNIQUE_ID'] ?>"
          ...
          data-validate
          data-onsubmit-trigger="servicesFormSubmit<?= $arResult['PREFIX_UNIQUE'] ?>"
    >
        <?= bitrix_sessid_post() ?>
...
<script>
    BX.ready(function() {
        class ServicesForm<?= $arResult['PREFIX_UNIQUE'] ?> extends UniversalForm {
            onSuccess(message) {
        ...
        let universalForm<?= $arResult['PREFIX_UNIQUE'] ?> = new ServicesForm<?= $arResult['PREFIX_UNIQUE'] ?>(<?= Json::encode($arResult['JS_PARAMS']) ?>);

        $(document).on('servicesFormSubmit<?= $arResult['PREFIX_UNIQUE'] ?>', function(event, form) {
